...
export default function EditImage() {
  ...
  useEffect(() => {
    // Fetch the image details from the server
    const fetchImageDetails = async () => {
      try {
        const response = await api.get(`/images/single/${id}`);
        loadImageFromUrl(response.data.imageUrl);
      } catch (error) {
  ...
  const loadImageFromUrl = (url: string) => {
    const img = new Image();
    img.crossOrigin = 'anonymous'; // Necessary if the image is served from a different origin
    img.onload = () => {
      setImage(img);
    };
    img.src = url;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await api.put(`/images/edit/${id}`, formData);
